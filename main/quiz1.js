(function ($, undefined) {
  ...
  var slotNumber = urlParams.get('slot');
...
$(document).ready(function () {
  ...
  var urlParams = new URLSearchParams(window.location.search);
        ...
        // Assign words based on slot number
        var words;
        if (slotNumber === '1') {
            words = "relay,tracks,foul,throw,smash,dive,punch,homerun,net,pull,hattrick,midon,sixer,goal,stump,strike"; // Concatenate words1 and words2
        } else if (slotNumber === '2') {
            words = "asmio,Artificial,gripper,tomservo,astro,qrio,aibo,bender,Simulation,chassis,bishop,toyota,gort,trajectory,marvin"; // Concatenate words1 and words2
        } else if (slotNumber === '3') {
            words = "Diwali,Holi,Pongal,Ramzan,Bakrid,Navaratri,Onam,Chirstmas,Easter,Halloween,Newyear,Ugadi,Sivaratri,Dasshera,Muharam"; // Concatenate words1 and words2
        } else if (slotNumber === '4') {
          words = "Appendix,Heart,Kidney,Intestine,Brain,Spine,Skull,Fingers,Stomach,Chest,Pancreas,Lungs,Tongue,Elbow,Shoulder"; // Concatenate words1 and words2
        } else if (slotNumber === '5') {
          words = "Elite,Friends,Dark,Victor,Witcher,Lucifer,Escape,Blacklist,Awake,Avengers,Matrix,After,Crown,Extinct,Plane"; // Concatenate words1 and words2
        } else if (slotNumber === '6') {
          words = "Cluster,image,neural,intel,caveats,recurrent,vector,linear,tasks,error,tools,tracking,analysis,wudao,aitocracy"; // Concatenate words1 and words2
        } else if (slotNumber === '7') {
          words = "Vostok,Brazil,Ecuador,Bolivia,Jamaica,Cuba,minsk,mali,iran,Belgium,Uruguay,togo,chad,atyrau,Haiti"; // Concatenate words1 and words2
        } else if (slotNumber === '8') {
          words = "Kafka,ernest,Tolkien,Wilde,Poe,Austen,roald,Eliot,Chaucer,Bronte,Yeats,Blake,Woolf,Borges,Rumi"; // Concatenate words1 and words2
        } else if (slotNumber === '9') {
          words = "fuchsia,Lily,Tulip,Daisy,dahlia,Orchid,Daffodil,Peony,Hyacinth,Carnation,Iris,Marigold,kudzu,Jasmine,Hibiscus"; // Concatenate words1 and words2
        } else if (slotNumber === '10') {
          words = "Fossil,Adidas,Walmart,Nike,Nokia,Canon,Trends,Toyota,Honda,Skechers,Philips,Verizon,Adobe,Sony,Hermes"; // Concatenate words1 and words2
        } else if (slotNumber === '11') {
          words = "Blackberry,Asus,Oppo,Oneplus,Honor,Huawei,Vivo,HTC,Poco,Motorola,Infinix,Tecno ,Redmi,Xiaomi,Samsung"; // Concatenate words1 and words2
        } else if (slotNumber === '12') {
          words = "pinenut,prunes,fig,dates,hazel,mulberry,baru,pili,saba,pecan,pista,badam,almonds,goober,pindar"; // Concatenate words1 and words2
        } else if (slotNumber === '13') {
          words = "whitney,cyrus,nikola,karl,graham,arkwright,issac,gabriel,galileo,guglielmo,clement,thales,louis,carl,eugene"; // Concatenate words1 and words2
        } else if (slotNumber === '14') {
          words = "floppy,firestick,MacBook,webcam,Kindle,Galaxy,Nexus,Surface,Roku,Alexa,Fitbit,Drone,obyte,Oculus,Earbuds"; // Concatenate words1 and words2
        } else if (slotNumber === '15') {
          words = "Chess,Poker,rugby,Scrabble,Solitaire,Checkers,gunning,buzkasti,Sudoku,soccer,Snake,Tetris,jetpac,Hangman,kameo"; // Concatenate words1 and words2
        } else if (slotNumber === '16') {
          words = "astral,Galaxy,Stars,martian,Meteor,Nebula,stellar,galactic,Comet,Solar,Cosmic,Satellite,Orion,lunar,celestial"; // Concatenate words1 and words2
        } else if (slotNumber === '17') {
          words = "Victoria,Albert,Edward,Alexandra,George,Mary,William,Charlotte,Henry,Elizabeth,Augustus,Sophia,Louise,Frederick,Adelaide"; // Concatenate words1 and words2
        } else if (slotNumber === '18') {
          words = "Processor,Memory,cloud,Graphics,Keyboard,Monitor,Mouse,Operating,System,Software,Virus,Backup,Program,byte,Cyber"; // Concatenate words1 and words2
        } else if (slotNumber === '19') {
          words = "Algorithm,Binary,Circuit,Database,Encrypt,Frequency,Hacking,Interface,avatar,Logic,attack,Node,Operate,Protocol,Quantum"; // Concatenate words1 and words2
        } else if (slotNumber === '20') {
          words = "link,signal,session,transport,recevier,connect,protocol,public,user,overlay,plug,client,cache,wifi,backup"; // Concatenate words1 and words2
        } else {
            alert("Invalid slot number. Redirecting to home page.");
            window.location.href = "1.html";
          }

        // Attach the game to the div with ID 'theGrid'
